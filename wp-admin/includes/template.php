...
function _post_states($post) {
	...
	if ( 'private' == $post->post_status && 'private' != $post_status )
		$post_states['private'] = __('Aguardando Pagamento');
	if ( 'draft' == $post->post_status && 'draft' != $post_status )
		$post_states['draft'] = __('Salve para Revisão');
	if ( 'pending' == $post->post_status && 'pending' != $post_status )
		/* translators: post state */
		$post_states['pending'] = _x('Aguardando Revisão', '');
	if ( is_sticky($post->ID) )
		$post_states['sticky'] = __('Sticky');
	if ( 'publish' == $post->post_status && 'publish' != $post_status )
		$post_states['publish'] = __('Aprovado');
	...
	if ( ! empty($post_states) ) {
		...
		foreach ( $post_states as $state ) {
			...
			echo "<br><span class='post-state'>$state</span>";
		}
	}
