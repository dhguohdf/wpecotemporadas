...
<article id="post-<?php the_ID(); ?>" <?php post_class( $status ); ?> itemscope itemtype="http://schema.org/RealEstateAgent">
	...
	<div class="entry-content clear" itemprop="description">
		...
		?>	
			...
			<div class="column large-8 small-8">
				<?php if( !empty( $agent_mobile ) || !empty( $agent_office) || !empty( $agent_fax) ) { ?>
				<div class="row">
	...
	<?php $show_contact = bon_get_option('show_agent_form', 'yes');
	if ( $show_contact == 'yes' ) { ?>
...
<?php $show_listing = bon_get_option('show_agent_listing', 'yes'); ?>

<?php if( $show_listing == 'yes' ) { ?>
