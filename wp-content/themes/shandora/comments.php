...
<section id="comments">
	...
	<?php if ( have_comments() ) : ?>
		...
		<?php if ( get_option( 'page_comments' ) && 1 < get_comment_pages_count() ) : ?>
			...
			<div class="comments-nav">
				<?php previous_comments_link( __( '&larr; Previous', 'bon' ) ); ?>
				...
				<?php next_comments_link( __( 'Next &rarr;', 'bon' ) ); ?>
			</div><!-- .comments-nav -->
	...
	<div id="respond">
		...
		<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
			...
			<p><?php printf(__('Logged in as %1$s. %2$sLog out &raquo;%3$s', 'bon'), '<a href="'.get_option('siteurl').'/wp-admin/profile.php">'.$user_identity.'</a>', '<a href="'.(function_exists('wp_logout_url') ? wp_logout_url(get_permalink()) : get_option('siteurl').'/wp-login.php?action=logout" title="').'" title="'.__('Log out of this account', 'bon').'">', '</a>') ?></p>
			...
			<div class="row collapse input-container">
				...
				<div class="column large-6 small-11">
					<input class="attached-input" type="text" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" size="22" tabindex="1" />
				</div>
			</div>
			...
			<div class="row collapse input-container">
				...
				<div class="column large-6 small-11">
					<input class="attached-input" type="text" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" size="22" tabindex="2" />
				</div>
			</div>
		
			<div class="row collapse input-container">
				<div class="column large-1 small-1"><span class="attached-label prefix"><i class="sha-globe"></i></span></div>
				<div class="column large-6 small-11">
					<input class="attached-input" type="text" name="url" id="url" value="<?php echo esc_attr($comment_author_url); ?>" size="22" tabindex="3" />
				</div>
			</div>
			...
			<div class="row collapse textarea-container input-container" data-match-height>
				...
				<div data-height-watch class="column large-11 small-11">
					<textarea name="comment" class="attached-input" id="comment" cols="58" rows="10" tabindex="4"></textarea>
				</div>
			</div>
			
			<!--<p class="allowed-tags"><small><strong>XHTML:</strong> You can use these tags: <code><?php echo allowed_tags(); ?></code></small></p>-->
		
			<div><input class="flat button red radius" name="submit" type="submit" id="submit" tabindex="5" value="<?php _e('Submit Comment', 'bon') ?>" />
