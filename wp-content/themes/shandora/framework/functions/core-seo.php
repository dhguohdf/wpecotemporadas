...
add_action( 'wp_head', 'bon_meta_robots', 1 );
add_action( 'wp_head', 'bon_meta_author', 1 );
add_action( 'wp_head', 'bon_meta_copyright', 1 );
add_action( 'wp_head', 'bon_meta_revised', 1 );
add_action( 'wp_head', 'bon_meta_description', 1 );
add_action( 'wp_head', 'bon_meta_keywords', 1 );

/**
 * Sets the default meta robots setting.  If private, don't send meta info to the header.  Runs the 
 * bon_meta_robots filter hook at the end.
 *
 * @since 1.0
 * @access public
 * @return void
 */
function bon_meta_robots() {
 ...
 * @return void
 */
function bon_meta_copyright() {

	/* If viewing a singular post, get the post month and year. */
	if ( is_singular() )
		$date = get_the_time( esc_attr__( 'F Y', 'bon' ) );

	/* For all other views, get the current year. */
	else
		$date = date( esc_attr__( 'Y', 'bon' ) );

	/* Create the HTML for the copyright meta tag. */
	$copyright = '<meta name="copyright" content="' . sprintf( esc_attr__( 'Copyright (c) %1$s', 'bon' ), $date ) . '" />' . "\n";
...
function bon_meta_revised() {
	...
	if ( is_singular() )
		$revised = '<meta name="revised" content="' . get_the_modified_time( esc_attr__( 'l, F jS, Y, g:i a', 'bon' ) ) . '" />' . "\n";
