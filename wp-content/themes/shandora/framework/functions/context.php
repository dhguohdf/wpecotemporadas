...
add_filter( 'body_class', 'bon_body_class_filter', 0, 2 );

/* Filters the WordPress 'post_class' early. */
add_filter( 'post_class', 'bon_post_class_filter', 0, 3 );

/* Filters the WordPress 'comment_class' early. */
add_filter( 'comment_class', 'bon_comment_class_filter', 0, 3 );
...
function bon_get_context() {
	...
	return array_map( 'esc_attr', apply_filters( 'bon_context', array_unique( $bon->context ) ) );
}
...
* @return array
*/
function bon_post_class_filter( $classes, $class, $post_id ) {
	global $wp_query;
    ...
    $remove = array( "type-{$post_type}", "status-{$post_status}", 'post-password-required' );
    ...
    if ( !is_singular() && $post_alt <= 1 ) {
		$_classes[] = 'post-first';
	}
	
    $_classes[] = $post_type;
    $_classes[] = $post_status;
    $_classes[] = ( $post_alt % 2 ) ? 'odd' : 'even alt';
...
* @return array
*/
function bon_comment_class_filter( $classes, $class, $comment_id ) {
