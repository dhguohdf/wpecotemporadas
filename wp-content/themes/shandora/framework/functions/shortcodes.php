...
function bon_loginout_link_shortcode() {
	if ( is_user_logged_in() )
		$out = '<a class="logout-link" href="' . esc_url( wp_logout_url( site_url( $_SERVER['REQUEST_URI'] ) ) ) . '" title="' . esc_attr__( 'Log out', 'bon' ) . '">' . __( 'Log out', 'bon' ) . '</a>';
	else
		$out = '<a class="login-link" href="' . esc_url( wp_login_url( site_url( $_SERVER['REQUEST_URI'] ) ) ) . '" title="' . esc_attr__( 'Log in', 'bon' ) . '">' . __( 'Log in', 'bon' ) . '</a>';
...
function bon_entry_edit_link_shortcode( $attr ) {
	...
	return $attr['before'] . '<span class="entry-edit-meta entry-post-meta"><a class="post-edit-link" href="' . esc_url( get_edit_post_link( get_the_ID() ) ) . '" title="' . sprintf( esc_attr__( 'Edit %1$s', 'bon' ), $post_type->labels->singular_name ) . '">' . __( 'Edit', 'bon' ) . '</a></span>' . $attr['after'];
}
...
function bon_entry_comments_link_shortcode( $attr ) {
	...
	$attr = shortcode_atts( array( 'zero' => __( 'Comment:', 'bon' ), 'one' => __( 'Comment:', 'bon' ), 'more' => __( 'Comments:', 'bon' ), 'css_class' => 'comments-link', 'none' => '', 'before' => '', 'after' => '' ), $attr );
...
function bon_entry_terms_shortcode( $attr ) {
	$attr = shortcode_atts( array( 'text' => 'Category:', 'exclude_child' => false, 'id' => get_the_ID(), 'limit' => -1, 'taxonomy' => 'post_tag', 'separator' => ', ', 'before' => '', 'after' => '' ), $attr );
	$termlists = '';
...
function bon_entry_permalink_shortcode( $attr ) {
	...
	return $attr['before'] . '<a href="' . esc_url( get_permalink() ) . '" class="permalink '.$attr['class'].'" title="'. the_title_attribute(array( 'before' => __('Permalink to ', 'bon'), 'echo' => 0)) .'">' . sprintf(__( '%s', 'bon' ), $attr['text'] ) . '</a>' . $attr['after'];
}
...
function bon_entry_icon_shortcode( $attr ) {
	...
	$o = '<a class="entry-post-meta entry-icon-meta '.$attr['class'].'" href="'.get_permalink().'" title="'.the_title_attribute( array( 'before' => __('Permalink to ', 'bon'), 'echo' => 0) ).'">';
