			...
			if($this->context == 'options_page') {
				...
				if ( count( $menu_items ) > 0 ) {
					...
					foreach ( $menu_items as $k => $v ) {
						...
						if ( isset( $v['icon'] ) && ( $v['icon'] != '' ) )
						$menu .= '<a title="' . esc_attr( $v['label'] ) . '" href="#' . $v['token'] . '"><i class="icon"></i>' . esc_html( $v['label'] ) . '</a>' . "\n";
			...
			switch( $type ) {
				...
				$value = $meta != '' ? intval( $meta ) : '0';
					$output .= '<div id="' . esc_attr( $id ) . '-slider" class="ui-slide" data-min="'.$min.'" data-max="'.$max.'" data-step="'.$step.'"></div>
							<input class="bon-input" type="text" name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '" value="' . $value . '" size="5" />
							<br />' . $desc;
				break;
				// image
				case 'image':
