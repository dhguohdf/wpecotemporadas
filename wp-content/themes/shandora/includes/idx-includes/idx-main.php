...
class Bon_IDX {
   ...
   static public $SearchSetupID = '';
	...
	protected function add_template_actions() {
		...
		// Actions header
        add_action('bon_idx_before_results_content', array( $this, 'pro_profile_header') );

        add_action('bon_idx_after_results_content', array( $this, 'pro_profile_footer') );
	...
	function pro_profile_header() {
        $this->idx_include( 'pro-profile-header.php', false );
	}

	function pro_profile_footer() {
		$this->idx_include( 'pro-profile-footer.php', false );
	}
	...
	function template_site_disclaimer() {
        ...
        if (!did_action('bon_idx_listing_source')) {
            do_action('bon_idx_listing_source');
        }
  ...
  <?php
	...
	function process_globals($content = null) {
		...
		if ($this->helper->is_listing()) {
			...
			$top_search = $this->helper->strip_spaces($qp->find('#dsidx-top-search')->xhtml());
	        $this->helper->set_global('pro_top_search', $top_search);
	        
	        $profile_header = $this->helper->strip_spaces($qp->find('#dsidx-profile-header')->xhtml());
	        $this->helper->set_global('pro_profile_header', $profile_header);
	        
	        $top_actions = $this->helper->strip_spaces($qp->find('.dsidx-top-actions')->xhtml());
	        $linktext    = str_replace('Save Search', __('Save Search','bon'), $top_actions);
	        $linktext    = str_replace('Save to Favorites', __('Save to Favorites','bon'), $linktext);
	        $this->helper->set_global('pro_top_actions', $linktext);
	        
	        preg_match($this->get_regex('pro_visitor_javascript'), $content, $matches);
	        if (isset($matches[1])) {
	            $this->helper->set_global('pro_visitor_javascript', "\n" . '<script>' . "\n" . $matches[1] . "\n" . '</script>');
	        }
	        
	        preg_match($this->get_regex('pro_search_tabs'), $content, $matches);
	        if (isset($matches[1])) {
	            $this->helper->set_global('pro_search_tabs', $matches[1]);
	        }
	        
	        preg_match($this->get_regex('pro_call_to_action'), $content, $matches);
	        if (isset($matches[1])) {
	            $this->helper->set_global('pro_call_to_action', $matches[1]);
	        }
	        
	        preg_match($this->get_regex('pro_midx_javascript'), $content, $matches);
	        $this->helper->set_global('pro_midx_javascript', isset($matches[1]) ? '<script>' . $matches[1] . '</script>' : '');
...
function is_sold() {
	...
	if (!empty($wp)) {
		...
		if (isset($wp->wp_idx_content['ListingStatuses']) && $wp->wp_idx_content['ListingStatuses'] == '8') {
			return true;
		}
	}
