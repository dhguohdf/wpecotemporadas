...
function shandora_search_manufacturer_level1_field($value = array(), $class) {
	...
	return apply_filters( 'shandora_search_manufacturer_level1_field_output', $o );
...
function shandora_search_manufacturer_level2_field($value = array(), $class) {
	...
	if($value['manufacturer_level1'] != '') {
		...
		if($parent) {
			$terms = get_terms('manufacturer', array('parent' => $parent->term_id));
		    if($terms) {
		    	foreach($terms as $term) {
			    	$loc_opt[$term->slug] = $term->name;
			    }
	...
	return apply_filters( 'shandora_search_manufacturer_level2_field_output', $o );
...
function shandora_search_manufacturer_level3_field($value = array(), $class) {
	...
	return apply_filters( 'shandora_search_manufacturer_level3_field_output', $o );
...
function shandora_ajax_update_location_level($slug) {
	...
	if(!empty($slug)) {
		...
		if($parent) {
			$terms = get_terms('property-location', array( 'hide_empty' => false, 'parent' => $parent->term_id));
		    if($terms) {
		    	foreach($terms as $term) {
			    	$loc_opt[$term->slug] = $term->name;
			    }
...
function shandora_ajax_update_manufacturer_level($slug) {
	...
	if(!empty($slug)) {
		...
		if($parent) {
			$terms = get_terms('manufacturer', array( 'hide_empty' => false, 'parent' => $parent->term_id));
		    if($terms) {
		    	foreach($terms as $term) {
			    	$loc_opt[$term->slug] = $term->name;
			    }
