	...
	function widget( $sidebar, $instance ) {
		...
		<div class="row">
			<div class="column large-12">
				<?php if ($instance['type'] == "realestate") { ?>
					<script type="text/javascript">
						(function(f){MortgageCalculator=function(){this.initialize.apply(this,arguments)};MortgageCalculator.rules={".mc-container":"cursor: default; text-align: left",".mc-title":"font-weight: bold;",".mc-clear":"clear: both",".mc-logo":"float: right; margin: 0 0 5px 0; border: 0",".mc-graph":"display: none; margin: 10px 0 0 0",
						".mc-legend":"display: inline-block; margin: 0 5px 0 0; line-height: 7pt; width: 7pt; height: 7pt",".mc-scroll":"margin: 0; overflow-x: hidden; overflow-y: scroll;",".mc-message":"",".mc-button":"cursor: pointer; float: right; width: 100%; height: 30px; padding: 1px; border: 1px solid; -moz-border-radius: 4px; -webkit-border-radius: 4px; font-weight: bold;",".mc-footer":"margin: 2px 0 0 0; text-align: right",
						".mc-navs":{self:"margin: 0; padding: 0; line-height: 25px; font-weight: bold; text-align: center",".mc-nav":"width: 25px; height: 25px; margin: 0; padding: 1px; border: 1px solid; -moz-border-radius: 3px; -webkit-border-radius: 3px; font-weight: bold"},".mc-small":"height: 25px; ",".mc-tables":"position: relative; overflow: hidden; height: 122px !important; margin: 10px 0 0 0; text-align: left",
						".mc-fields":{self:"clear:both; overflow:hidden; margin: 0 10px 0 0","*":"float: left",label:"width: 110px; line-height: 20px; font-weight: bold; text-align: left",input:"margin: 0 0 5px 0; padding: 2px; width: 90px; border: 1px solid; text-align: right",select:"margin: 0 0 5px 0; padding: 2px; width: 96px; border: 1px solid; ",div:"padding: 0 0 0 5px; width: 25px; line-height: 20px; text-align: left",
						...
						li:"padding: 1em 0 0 0"},".mc-table":{self:"table-layout: fixed; width: 100%; border-collapse: collapse",th:"padding: 0 0 2px 0; font-size: 9pt; font-weight: bold; text-align: center",td:"padding: 0; line-height: 2em; border: 1px solid; font-size: 9pt; text-align: center",sup:"font-family: Verdana, Arial, san-serif; vertical-align: top"},".mc-widget":{".mc-title":"text-align: center",".mc-body":"",
						".mc-logo":"float: none; text-align: center",".mc-button":"float: none; clear: left; width: 100%; margin: 5px 0 0 0",".mc-footer":"text-align: left",".mc-fields":{self:"margin: 0",label:"margin: 0 0 0 2px; width: 95px; line-height: 18px;  font-size: 9pt",input:"margin: 0 0 4px 0; width: 166px; ",select:"margin: 0 0 4px 0; width: 72px;",
						div:"padding: 0 0 0 4px; width: 18px; line-height: 18px;  font-size: 7pt"},".mc-panes":{".mc-pane":"position: static; top: 0; margin: 10px 0 0 0; padding: 5px; -moz-border-radius: 5px; -webkit-border-radius: 5px"},".mc-tab":"line-height: 28px; margin: 0; padding: 0 5px 0 5px;   font-weight: bold",".mc-table":{self:"position: absolute",th:"padding: 0;  font-size: 9pt; text-align: left",td:"padding: 0 2px 0 2px; line-height: 2em; text-align: right"}}};
						...
						l){for(;l++<=h.input.term;)i.push(l);return i.join("|")}([],0)+"|1:|$0|$"+k.month.total.toFixed(0),chf:"bg,s,"+e.pane.back.substr(1),chco:a.C(g),chd:"e"+g.length+":"+(m.extra>0?a.E(A.extra,o)+",":"")+a.E(A.principal,o)+","+a.E(A.interest,o)+(m.taxes>0?","+a.E(A.taxes,o):"")+(m.pmi>0||m.insurance>0?","+a.E(A.insurance,o):"")+","+a.E(A.balance)};s.processed=true}},validate:function(){V=function(d,n,j,m,k){if(b.fields[d].enabled){d=e[d];var g=
						...
						(F=z.toString().split("."))[1]?F[1].length:0;p<=m.max[r];p+=z)f("<option/>").val(p).text(b.N(p).toFixed(F)).appendTo(l)}f("<div/>").text(D||"").appendTo(i);f("<br/>").appendTo(i)};button=function(i,l){var r=b.browser;r=r.msie||r.opera;return f("<button/>").addClass("button flat radius").css(d.css.button).text(l).mousedown(r?b.F:function(p){f(p.target).addClass("mc-pressed")}).mouseup(r?b.F:function(p){f(p.target).removeClass("mc-pressed")}).appendTo(i).click(function(p){p.preventDefault()})};navMC=function(i,
						...
						f.each(A,function(z){r[this]=b.cell(p).css({background:l=="total"||z==3?d.value.total:d.table.back,borderLeftWidth:z==3?2:1,borderTopWidth:l=="total"?2:1})})});e.legend=h.find(".mc-legend");e.message=f("<div/>").addClass("mc-message").css("padding","4px 0 0 70px").appendTo(h);h=pane(g,"amort",false,function(){if(!this.shown){var i=b.browser,
						...
						"Print").addClass("mc-small").click(b.B(b,b.amort,true));c.legend=h.find(".mc-legend");c.message=f("<div/>").addClass("mc-message").appendTo(h);}b.footer&&f("<div/>").addClass("mc-footer").css(d.css.footer).html(b.footer).appendTo(a.obj);g=a.obj;if(s.msie){g.find("p:first").css("margin","0");g.find(".mc-nav").css("line-height","20px");b.quirks&&g.find("input").width(g.find("select").width()+
