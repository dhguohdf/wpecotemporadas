...
switch ($column_name) {
case 'pagamento' :
$EmailVendedor = $opt['rwp_pagseguro_email'];
...
echo '<form action="https://sandbox.pagseguro.uol.com.br/v2/checkout/payment.html" method="post" onsubmit="PagSeguroLightbox(this); return false;">
		    ...
		    <input type="hidden" name="code" value="' . $opt['rwp_pagseguro_token'] . '">
...
function ecotemporadas_login_stylesheet() {
	...
	<link rel="stylesheet" id="custom_wp_admin_css"
	      href="<?php echo get_bloginfo( 'stylesheet_directory' ) . '/style-login.css'; ?>" type="text/css"
	      media="all"/>
<?php
}
