...
function shandora_get_rent_period($price) {
	...
	$period = shandora_get_meta($post->ID, 'listing_period'); 
		...
		switch ($period) {
			case 'per-diaria':
				$price .= ' <span class="rent-period">/'.__('diária','bon').'</span>';
			break;

			case 'per-temp':
				$price .= ' <span class="rent-period">/'.__('temporada','bon').'</span>';
			break;

			case 'per-noite':
				$price .= ' <span class="rent-period">/'.__('pernoite','bon').'</span>';
			break;
			
			case 'per-mensal':
				$price .= ' <span class="rent-period">/'.__('mês','bon').'</span>';
			break;
...
function shandora_get_search_listing_form() {
	...
	$output = $form->form_open( get_permalink($search_page), 'method="get" class="custom" id="search-listing-form"' );

	$output .= $ro . '<div class="column large-10 small-12 large-uncentered small-centered">';
