...
function shandora_render_builder_element_agent($value) {
	...
	$tw = get_post_meta( $post_id, $prefix . 'agentgplus', true );
	...
	if(!empty($tw)) {
		$o .= '<li><a href="'.$tw.'" title="Google+"><i class="awe-google-plus icon"></i></a></li>';
	}
	if(!empty($fb)) {
		$o .= '<li><a href="'.$fb.'" title="Facebook"><i class="awe-facebook icon"></i></a></li>';
	} 
	if(!empty($li)) {
		$o .= '<li><a href="'.$li.'" title="LinkedIn"><i class="awe-linkedin icon"></i></a></li>';
	} 
	$o .= '</ul></div>';
	$o .= '</section>';
	return $o;
	...
	$o .= '<ul class="listings '.$ul_class.'" data-compareurl="'.get_permalink($compare_page).'">';
		...
		while ( have_posts() ) : the_post();
			...
			$o .= '<article id="post-'.get_the_ID().'" class="'.bon_entry_class($status, null, false).'" itemscope itemtype="http://schema.org/RealEstateAgent">';
				...
				if(empty($bed)) { $o .= __('-','bon'); } else { $o .= sprintf( _n('%s Quarto','%s Quartos', $bed , 'bon'), $bed ); }
				
				$o .= '</span></div>';

				$o .= '<div class="icon bath"><i class="sha-bath"></i><span>';

				if(empty($bath)) { $o .= __('-','bon'); } else { $o .= sprintf(_n('%s Banheiro','%s Banheiros', $bath , 'bon'), $bath ); }
				$o .= '</span></div>';

				
				$o .= '<div class="icon bed"><i class="sha-ruler"></i><span>';
				if($lotsize) { $o .= $lotsize . ' ' . $sizemeasurement; } else { $o .= __('-','bon'); }
				$o .= '</span></div>';

				$o .= '<div class="icon bath"><i class="awe-money"></i><span>';

					$price = shandora_get_meta(get_the_ID(), 'listing_price', true); 
				if(empty($price)) { $o .= __('-','bon'); } else { $o .= shandora_get_listing_price(false); }
				$o .= '</span></div>';
				...
				$o .= '<div class="property-price"><a href="'.get_permalink( $post_id ).'" title="'.the_title_attribute( array('before' => __('Clique para entrar em ', 'bon'), 'echo' => false) ).'">'. $loca .'</a></div></footer><!-- .entry-footer -->';
	...
	$o .= '<ul class="listings '.$ul_class.'" data-compareurl="'.get_permalink($compare_page).'">';
		...
		while ( have_posts() ) : the_post();
			...
			$o .= '<article id="post-'.get_the_ID().'" class="'.bon_entry_class($badgeclr, null, false).'" itemscope itemtype="http://schema.org/RealEstateAgent">';
				...
				if($engine){ $o .= $engine; } else { $o .= __('Não especificado','bon'); }
				
				$o .= '</span></div>';

				$o .= '<div class="icon transmission"><i class="sha-gear-shifter"></i><span>';

				if($transmission){ $o .= $transmission; } else { $o .= __('Não especificado','bon'); }

				$o .= '</span></div>';

				$o .= '<div class="icon mileage"><i class="awe-dashboard"></i><span>';
				if($mileage) { $o .= $mileage; } else { $o .= __('Não especificado','bon'); }
				$o .= '</span></div>';
...
function shandora_override_contact_form_widget($output, $email) {
    ...
    $o .= '<input type="text" value="" name="name" class="name attached-input required" placeholder="'.__('Nome', 'bon').'" />';
    $o .= '<div class="contact-form-error">'.__('Por favor, insira seu nome.','bon-toolkit').'</div>';
    $o .= '</div>';
    $o .= '</div>';
    ...
    $o .= '</div>';
    $o .= '<div class="column large-10 small-11">';
    $o .= '<input type="email" value="" name="email" class="email-address attached-input required" placeholder="'.__('Email', 'bon').'" />';
    $o .= '<div class="contact-form-error">'.__('Por favor, inserir seu email.','bon-toolkit').'</div>';
    $o .= '</div>';
    $o .= '</div>';
    ...
    $o .= '</div>';
    $o .= '<div class="column large-10 small-11">';
    $o .= '<input type="text" value="" name="subject" class="subject" placeholder="'.__('Assunto', 'bon').'" />';
    $o .= '</div>';
    $o .= '</div>';
    ...
    $o .= '<textarea name="messages" class="messages required" placeholder="'.__('Mensagem', 'bon').'"></textarea>';
    $o .= '<div class="contact-form-error">'.__('Por favor, digite a sua mensagem.','bon-toolkit').'</div>';
    $o .= '</div>';
    $o .= '</div>';
    ...
    $o .= '<button type="submit" class="contact-form-submit flat button red radius" name="submit">'.__('Enviar mensagem','bon-toolkit').'</button>';
