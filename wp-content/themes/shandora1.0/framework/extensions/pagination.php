...
function bon_page_navi($args = array()) {
    ...
    if ($start_page >= 2 && $pages_to_show < $max_page) {
        ...
        $output .= '<li class="bon-first-page-link '.$first_class.'"><a href="'.get_pagenum_link().'" title="'.$first_page_text.'"><i class="awe-double-angle-left"></i></a></li>';
    }

    if(get_previous_posts_link()) {
        $output .= '<li class="bon-prev-link">';
        $output .= get_previous_posts_link(__('Anterior','bon'));
        $output .= '</li>';
    }
    else {
        $output .= '<li class="'.$disabled_class.'"><a href="#">' . __('Anterior', 'bon') . '</a>';
    }
    ...
    if(get_next_posts_link())  {
        ...
        $output .= get_next_posts_link(__('Próximo','bon'));
        $output .= '</li>';
    }
    else {
        $output .= '<li class="'.$disabled_class.'"><a href="#">' . __('Próximo', 'bon') . '</a>';
    }

    if ($end_page < $max_page) {
        $last_page_text = __('Last','bon');
        $output .= '<li class="bon-last-page-link '.$last_class.'"><a href="'.get_pagenum_link($max_page).'" title="'.$last_page_text.'"><i class="awe-double-angle-right"></i></a></li>';
    }
    $output .= '</ul></div>'.$after."";
