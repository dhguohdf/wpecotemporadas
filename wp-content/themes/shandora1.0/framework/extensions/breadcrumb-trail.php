 ...
 */
function bon_breadcrumb_trail( $args = array() ) {
	...
	$defaults = array(
		...
		'separator'  => '/',
		'before'     => __( 'Browse:', 'bon' ),
		'after'      => false,
		'front_page' => true,
		'show_home'  => __( 'Home', 'bon' ),
		'network'    => false,
		'echo'       => true,
		'use_separator' => true,
	);
	...
	$args = apply_filters( 'bon_breadcrumb_trail_args', $args );

	/* Parse the arguments and extract them for easy variable naming. */
	$args = wp_parse_args( $args, $defaults );

	/* Get the trail items. */
	$trail = bon_breadcrumb_trail_get_items( $args );
	...
	$breadcrumb = apply_filters( 'bon_breadcrumb_trail', $breadcrumb, $args );
 ...
 */
function bon_breadcrumb_trail_get_items( $args = array() ) {
	global $wp_rewrite;
	...
	if ( function_exists( 'is_bbpress' ) && is_bbpress() ) {
		$trail = array_merge( $trail, bon_breadcrumb_trail_get_bbpress_items() );
	}
	...
	elseif ( is_home() ) {
		...
		$trail = array_merge( $trail, bon_breadcrumb_trail_get_parents( $home_page->post_parent, '' ) );
	...
	elseif ( is_singular() ) {
		...
		if ( 'post' == $post_type ) {
			...
			if ( !empty( $path ) )
				$trail = array_merge( $trail, bon_breadcrumb_trail_get_parents( '', $path ) );

			/* Map the permalink structure tags to actual links. */
			$trail = array_merge( $trail, bon_breadcrumb_trail_map_rewrite_tags( $post_id, get_option( 'permalink_structure' ), $args ) );
		}
		...
		elseif ( 'attachment' == $post_type ) {
			...
			if ( 0 < $parent_id ) {
				...
				if ( 'post' == $parent_post_type ) {
					...
					$trail = array_merge( $trail, bon_breadcrumb_trail_map_rewrite_tags( $post->post_parent, get_option( 'permalink_structure' ), $args ) );
				}
		...
		if ( ( empty( $path ) && 0 !== $parent ) || ( 'attachment' == $post_type ) )
			$trail = array_merge( $trail, bon_breadcrumb_trail_get_parents( $parent, '' ) );
	...
	elseif ( is_archive() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
			...
			if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent )
				$trail = array_merge( $trail, bon_breadcrumb_trail_get_term_parents( $term->parent, $term->taxonomy ) );
		...
		elseif ( is_time() ) {
			...
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$trail[] = get_the_time( __( 'g:i a', 'bon' ) );

			elseif ( get_query_var( 'minute' ) )
				$trail[] = sprintf( __( 'Minute %1$s', 'bon' ), get_the_time( __( 'i', 'bon' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$trail[] = get_the_time( __( 'g a', 'bon' ) );
		}
		...
		elseif ( is_date() ) {
			...
			if ( $wp_rewrite->front )
				$trail = array_merge( $trail, bon_breadcrumb_trail_get_parents( '', $wp_rewrite->front ) );

			if ( is_day() ) {
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'bon' ) ) . '">' . get_the_time( __( 'Y', 'bon' ) ) . '</a>';
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . get_the_time( esc_attr__( 'F', 'bon' ) ) . '">' . get_the_time( __( 'F', 'bon' ) ) . '</a>';

				if ( is_paged() )
					$trail[] = '<a href="' . get_day_link( get_the_time( 'Y' ), get_the_time( 'm' ), get_the_time( 'd' ) ) . '" title="' . get_the_time( esc_attr__( 'd', 'bon' ) ) . '">' . get_the_time( __( 'd', 'bon' ) ) . '</a>';
				else
					$trail[] = get_the_time( __( 'd', 'bon' ) );
			}
			...
			elseif ( get_query_var( 'w' ) ) {
				...
				if ( is_paged() )
					$trail[] = get_archives_link( add_query_arg( array( 'm' => get_the_time( 'Y' ), 'w' => get_the_time( 'W' ) ), home_url() ), sprintf( __( 'Week %1$s', 'bon' ), get_the_time( esc_attr__( 'W', 'bon' ) ) ), false );
				else
					$trail[] = sprintf( __( 'Week %1$s', 'bon' ), get_the_time( esc_attr__( 'W', 'bon' ) ) );
			}
			...
			elseif ( is_month() ) {
				...
				if ( is_paged() )
					...
					$trail[] = get_the_time( __( 'F', 'bon' ) );
			}

			elseif ( is_year() ) {

				if ( is_paged() )
					$trail[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . esc_attr( get_the_time( __( 'Y', 'bon' ) ) ) . '">' . get_the_time( __( 'Y', 'bon' ) ) . '</a>';
				else
					$trail[] = get_the_time( __( 'Y', 'bon' ) );
			}
		}
	}
	...
	elseif ( is_search() ) {
		...
		if ( is_paged() )
			$trail[] = '<a href="' . get_search_link() . '" title="' . sprintf( esc_attr__( 'Search results for &quot;%1$s&quot;', 'bon' ), esc_attr( get_search_query() ) ) . '">' . sprintf( __( 'Search results for &quot;%1$s&quot;', 'bon' ), esc_attr( get_search_query() ) ) . '</a>';
		else
			$trail[] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'bon' ), esc_attr( get_search_query() ) );
	}

	/* If viewing a 404 error page. */
	elseif ( is_404() ) {
		$trail[] = __( '404 Not Found', 'bon' );
	}

	/* Check for pagination. */
	if ( is_paged() )
		$trail[] = sprintf( __( 'Page %d', 'bon' ), absint( get_query_var( 'paged' ) ) );
	elseif ( is_singular() && 1 < get_query_var( 'page' ) )
		$trail[] = sprintf( __( 'Page %d', 'bon' ), absint( get_query_var( 'page' ) ) );

	/* Allow devs to step in and filter the $trail array. */
	return apply_filters( 'bon_breadcrumb_trail_items', $trail, $args );
}
 ...
 * @return array List of items to be shown in the trail.
 */
function bon_breadcrumb_trail_get_bbpress_items( $args = array() ) {
	...
	elseif ( bbp_is_single_topic() ) {
		...
		$trail = array_merge( $trail, bon_breadcrumb_trail_get_parents( bbp_get_topic_forum_id( $topic_id ) ) );
	...
	elseif ( bbp_is_single_reply() ) {
		...
		$trail = array_merge( $trail, bon_breadcrumb_trail_get_parents( bbp_get_reply_topic_id( $reply_id ) ) );
	...
	elseif ( bbp_is_single_forum() ) {
		...
		if ( 0 !== $forum_parent_id)
			$trail = array_merge( $trail, bon_breadcrumb_trail_get_parents( $forum_parent_id ) );
	...
	return apply_filters( 'bon_breadcrumb_trail_get_bbpress_items', $trail, $args );
}
 ...
 */
function bon_breadcrumb_trail_map_rewrite_tags( $post_id = '', $path = '', $args = array() ) {
	...
	if ( is_array( $matches ) ) {
		...
		foreach ( $matches as $match ) {
			...
			if ( '%year%' == $tag )
				$trail[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'Y', 'bon' ), $post_id ) . '">' . get_the_time( __( 'Y', 'bon' ), $post_id ) . '</a>';

			/* If using the %monthnum% tag, add a link to the monthly archive. */
			elseif ( '%monthnum%' == $tag )
				$trail[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'bon' ), $post_id ) . '">' . get_the_time( __( 'F', 'bon' ), $post_id ) . '</a>';

			/* If using the %day% tag, add a link to the daily archive. */
			elseif ( '%day%' == $tag )
				$trail[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'bon' ), $post_id ) . '">' . get_the_time( __( 'd', 'bon' ), $post_id ) . '</a>';
			...
			elseif ( '%category%' == $tag && 'category' !== $args["singular_{$post->post_type}_taxonomy"] ) {
				...
				if ( $terms ) {
					...
					if ( 0 !== $term->parent )
						$trail = array_merge( $trail, bon_breadcrumb_trail_get_term_parents( $term->parent, 'category' ) );
 ...
 */
function bon_breadcrumb_trail_get_parents( $post_id = '', $path = '' ) {
 ...
 */
function bon_breadcrumb_trail_get_term_parents( $parent_id = '', $taxonomy = '' ) {
