...
add_action( 'wp_loaded', 'bon_structured_post_formats', 1 );

/* Filter the post format archive title. */
add_filter( 'single_term_title', 'bon_single_post_format_title' );
 ...
 * @return string
 */
function bon_single_post_format_title( $title ) {

	if ( is_tax( 'post_format' ) ) {
		$term   = get_queried_object();
		$plural = bon_get_plural_post_format_string( $term->slug );
 ...
 * @return string
 */
function bon_get_plural_post_format_string( $slug ) {

	$strings = bon_get_plural_post_format_strings();

	$slug = bon_clean_post_format_slug( $slug );
 ...
 * @return array
 */
function bon_get_plural_post_format_strings() {

	$strings = array(
	//	'standard' => __( 'Articles',       'bon' ), // Would this ever be used?
		'aside'    => __( 'Asides',         'bon' ),
		'audio'    => __( 'Audio',          'bon' ), // Leave as "Audio"?
		'chat'     => __( 'Chats',          'bon' ),
		'image'    => __( 'Images',         'bon' ),
		'gallery'  => __( 'Galleries',      'bon' ),
		'link'     => __( 'Links',          'bon' ),
		'quote'    => __( 'Quotes',         'bon' ), // Use "Quotations"?
		'status'   => __( 'Status Updates', 'bon' ),
		'video'    => __( 'Videos',         'bon' ),
	);

	return apply_filters( 'bon_plural_post_format_strings', $strings );
}
 ...
 * @return string
 */
function bon_clean_post_format_slug( $slug ) {
 ...
 */
function bon_aside_infinity( $content ) {
 ...
 * bon_get_gallery_image_count() function since it will count all gallery items regardless of mime type.
 *
 ...
 * @return int
 */
function bon_get_gallery_item_count() {
 ...
 * @return int
 */
function bon_get_gallery_image_count() {
 ...
 * @return string
 */
function bon_image_content( $content ) {
 ...
 * @return string
 */
function bon_get_content_url( $content ) {
 ...
 * @return string
 */
function bon_get_the_post_format_url( $url = '', $post = null ) {

	if ( empty( $url ) ) {

		$post = is_null( $post ) ? get_post() : $post;

		$content_url = bon_get_content_url( $post->post_content );
 ...
 * @return string $content
 */
function bon_link_content( $content ) {
 ...
 * @return string $content
 */
function bon_quote_content( $content ) {
 ...
 * @return array
 */
function bon_get_the_post_format_chat( $content ) {

	/* Allow the separator (separator for speaker/text) to be filtered. */
	$separator = apply_filters( 'bon_post_format_chat_separator', ':' );
 ...
 * @since  1.6.0
 * @access public
 * @global array   $_bon_post_chat_ids  An array of IDs for the chat rows based on the author.
 * @param  string  $content                The content of the post.
 * @return string  $chat_output            The formatted content of the post.
 */
function bon_chat_content( $content ) {
	...
	$stanzas = bon_get_the_post_format_chat( $content );
	...
	foreach ( $stanzas as $stanza ) {
		...
		foreach ( $stanza as $row ) {
			...
			$speaker_id = bon_chat_row_id( $chat_author );
			...
			if ( !empty( $chat_author ) )
				$chat_output .= "\n\t\t\t\t\t" . '<div class="chat-author ' . sanitize_html_class( strtolower( "chat-author-{$chat_author}" ) ) . ' vcard">' . $time . '<cite class="fn">' . apply_filters( 'bon_post_format_chat_author', $chat_author, $speaker_id ) . '</cite>:</div>';

			/* Add the chat row text. */
			$chat_output .= "\n\t\t\t\t\t" . '<div class="chat-text">' . str_replace( array( "\r", "\n", "\t" ), '', apply_filters( 'bon_post_format_chat_text', $chat_text, $chat_author, $speaker_id ) ) . '</div>';
 ...
 */
function bon_chat_row_id( $chat_author ) {
	global $_bon_post_chat_ids;

	/* Let's sanitize the chat author to avoid craziness and differences like "John" and "john". */
	$chat_author = strtolower( strip_tags( $chat_author ) );

	/* Add the chat author to the array. */
	$_bon_post_chat_ids[] = $chat_author;

	/* Make sure the array only holds unique values. */
	$_bon_post_chat_ids = array_unique( $_bon_post_chat_ids );

	/* Return the array key for the chat author and add "1" to avoid an ID of "0". */
	return absint( array_search( $chat_author, $_bon_post_chat_ids ) ) + 1;
}
