...
function custom_field_series( $args = array() ) {
	...
	if ( !empty( $meta_value ) ) {
		...
		if ( !empty( $series_posts ) ) {
			...
			$series .= '<h4 class="series-title">' . apply_filters( 'custom_field_series_title', __( 'Articles in this series', 'bon' ) ) . '</h4>';
			$series .= '<ul>';
...
function custom_field_series_create_meta_box( $post_type, $post ) {
	...
	if ( post_type_supports( $post_type, 'custom-field-series' ) )
		add_meta_box( 'custom-field-series', __( 'Series', 'bon' ), 'custom_field_series_meta_box', $post_type, 'side', 'default' );
}
