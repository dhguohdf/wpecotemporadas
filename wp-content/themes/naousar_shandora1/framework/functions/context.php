 ...
 * @global $bon The global Framework object.
 * @return array $bon->context Several contexts based on the current page.
 */
function bon_get_context() {
	global $bon;

	/* If $bon->context has been set, don't run through the conditionals again. Just return the variable. */
	if ( isset( $bon->context ) )
		return apply_filters( 'bon_context', $bon->context );
	...
	/* Singular views. */
	elseif ( is_singular() ) {
		$bon->context[] = 'singular';
		$bon->context[] = "singular-{$object->post_type}";
		$bon->context[] = "singular-{$object->post_type}-{$object_id}";
	}
	...
	elseif ( is_archive() ) {
		...
		if ( is_post_type_archive() ) {
			...
			$bon->context[] = "archive-{$post_type->name}";
		}

		/* Taxonomy archives. */
		if ( is_tax() || is_category() || is_tag() ) {
			$bon->context[] = 'taxonomy';
			$bon->context[] = "taxonomy-{$object->taxonomy}";

			$slug = ( ( 'post_format' == $object->taxonomy ) ? str_replace( 'post-format-', '', $object->slug ) : $object->slug );
			$bon->context[] = "taxonomy-{$object->taxonomy}-" . sanitize_html_class( $slug, $object->term_id );
		}
		...
		if ( is_author() ) {
			...
			$bon->context[] = 'user';
			$bon->context[] = 'user-' . sanitize_html_class( get_the_author_meta( 'user_nicename', $user_id ), $user_id );
		}
	...
	/* Error 404 pages. */
	elseif ( is_404() ) {
		$bon->context[] = 'error-404';
	}

	return array_map( 'esc_attr', apply_filters( 'bon_context', $bon->context ) );
}
...
function bon_entry_class( $class = '', $post_id = null, $echo = true ) {
	...
	if ( !empty( $post ) ) {
		...
		if(!is_singular() && $wp_query->post_count == $post_alt ) {
			$classes[] = 'post-last';
		}
	}
	...
	if($echo) {
		...
		return apply_atomic( 'entry_class', $class );
	}
}
...
function bon_comment_class( $class = '' ) {
	...
	echo apply_filters( "{$bon->prefix}comment_class", $class );
}
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date () ) {
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$doctitle = sprintf( __( 'Archive for %1$s', 'bon' ), get_the_time( __( 'g:i a', 'bon' ) ) );

			elseif ( get_query_var( 'minute' ) )
				$doctitle = sprintf( __( 'Archive for minute %1$s', 'bon' ), get_the_time( __( 'i', 'bon' ) ) );

			elseif ( get_query_var( 'hour' ) )
				$doctitle = sprintf( __( 'Archive for %1$s', 'bon' ), get_the_time( __( 'g a', 'bon' ) ) );

			elseif ( is_day() )
				$doctitle = sprintf( __( 'Archive for %1$s', 'bon' ), get_the_time( __( 'F jS, Y', 'bon' ) ) );

			elseif ( get_query_var( 'w' ) )
				$doctitle = sprintf( __( 'Archive for week %1$s of %2$s', 'bon' ), get_the_time( __( 'W', 'bon' ) ), get_the_time( __( 'Y', 'bon' ) ) );

			elseif ( is_month() )
				$doctitle = sprintf( __( 'Archive for %1$s', 'bon' ), single_month_title( ' ', false) );

			elseif ( is_year() )
				$doctitle = sprintf( __( 'Archive for %1$s', 'bon' ), get_the_time( __( 'Y', 'bon' ) ) );
		}

		/* For any other archives. */
		else {
			$doctitle = __( 'Archives', 'bon' );
		}
	}

	/* If viewing a search results page. */
	elseif ( is_search() )
		$doctitle = sprintf( __( 'Search results for &quot;%1$s&quot;', 'bon' ), esc_attr( get_search_query() ) );

	/* If viewing a 404 not found page. */
	elseif ( is_404() )
		$doctitle = __( '404 Not Found', 'bon' );

	/* If the current page is a paged page. */
	if ( ( ( $page = $wp_query->get( 'paged' ) ) || ( $page = $wp_query->get( 'page' ) ) ) && $page > 1 )
		$doctitle = sprintf( __( '%1$s Page %2$s', 'bon' ), $doctitle . $separator, number_format_i18n( $page ) );
