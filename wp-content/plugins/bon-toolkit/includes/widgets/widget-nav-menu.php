...
add_action( 'widgets_init', 'load_bon_toolkit_nav_menu_widget' );

function load_bon_toolkit_nav_menu_widget() {
	register_widget( 'Bon_Toolkit_Widget_Nav_Menu' );
}

class Bon_Toolkit_Widget_Nav_Menu extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'nav-menu',
			'description' => esc_html__( 'An advanced widget that gives you total control over the output of your menus.', 'bon-toolkit' )
		);
		...
		/* Create the widget. */
		$this->WP_Widget(
			'bon-toolkit-nav-menu',                      // $this->id_base
			BON_TOOLKIT_PREFIX . ' ' . __( 'Navigation Menu', 'bon-toolkit' ), // $this->name
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title'           => esc_attr__( 'Navigation', 'bon-toolkit' ),
			'menu'            => '',
			'container'       => 'div',
			'container_id'    => '',
			'container_class' => '',
