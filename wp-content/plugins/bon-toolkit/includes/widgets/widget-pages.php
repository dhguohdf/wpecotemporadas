...
add_action( 'widgets_init', 'load_bon_toolkit_pages_widget' );

function load_bon_toolkit_pages_widget() {
	register_widget( 'Bon_Toolkit_Widget_Pages' );
}

class Bon_Toolkit_Widget_Pages extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'pages',
			'description' => esc_html__( 'An advanced widget that gives you total control over the output of your page links.', 'bon-toolkit' )
		);
		...
		/* Create the widget. */
		$this->WP_Widget(
			'bon-toolkit-pages',              // $this->id_base
			BON_TOOLKIT_PREFIX . ' ' . __( 'Pages', 'bon-toolkit'), // $this->name
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title'        => esc_attr__( 'Pages', 'bon-toolkit'),
			'post_type'    => 'page',
			'depth'        => 0,
			'number'       => '',
			'offset'       => '',
		...
		$sort_column = array( 
			'post_author'   => esc_attr__( 'Author', 'bon-toolkit' ), 
			'post_date'     => esc_attr__( 'Date', 'bon-toolkit' ), 
			'ID'            => esc_attr__( 'ID', 'bon-toolkit' ), 
			'menu_order'    => esc_attr__( 'Menu Order', 'bon-toolkit' ), 
			'post_modified' => esc_attr__( 'Modified', 'bon-toolkit' ), 
			'post_name'     => esc_attr__( 'Slug', 'bon-toolkit' ), 
			'post_title'    => esc_attr__( 'Title', 'bon-toolkit' ) 
		);

		$show_date = array( 
			''         => '', 
			'created'  => esc_attr__( 'Created', 'bon-toolkit' ), 
			'modified' => esc_attr__( 'Modified', 'bon-toolkit' ) 
		);
