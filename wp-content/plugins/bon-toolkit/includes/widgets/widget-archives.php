...
add_action( 'widgets_init', 'load_bon_toolkit_archive_widget' );

function load_bon_toolkit_archive_widget() {
	register_widget( 'Bon_Toolkit_Widget_Archives' );
}

class Bon_Toolkit_Widget_Archives extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'archives',
			'description' => esc_html__( 'An advanced widget that gives you total control over the output of your archives.', 'bon-toolkit' )
		);
		...
		/* Create the widget. */
		$this->WP_Widget(
			'bon-toolkit-archives',               // $this->id_base
			BON_TOOLKIT_PREFIX . ' ' . __( 'Archives', 'bon-toolkit' ), // $this->name
	...
	function widget( $sidebar, $instance ) {
		...
		if ( 'option' == $args['format'] ) {
			...
			if ( 'yearly' == $args['type'] )
				$option_title = esc_html__( 'Select Year', 'bon-toolkit' );

			elseif ( 'monthly' == $args['type'] )
				$option_title = esc_html__( 'Select Month', 'bon-toolkit' );

			elseif ( 'weekly' == $args['type'] )
				$option_title = esc_html__( 'Select Week', 'bon-toolkit' );

			elseif ( 'daily' == $args['type'] )
				$option_title = esc_html__( 'Select Day', 'bon-toolkit' );

			elseif ( 'postbypost' == $args['type'] || 'alpha' == $args['type'] )
				$option_title = esc_html__( 'Select Post', 'bon-toolkit' );
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title'           => esc_attr__( 'Archives', 'bon-toolkit' ),
			'limit'           => 10,
			'type'            => 'monthly',
			'order'           => 'DESC',
			'format'          => 'html',
		...
		$type = array( 
			'alpha'      => esc_attr__( 'Alphabetical', 'bon-toolkit' ), 
			'daily'      => esc_attr__( 'Daily', 'bon-toolkit' ), 
			'monthly'    => esc_attr__( 'Monthly', 'bon-toolkit' ),
			'postbypost' => esc_attr__( 'Post By Post', 'bon-toolkit' ), 
			'weekly'     => esc_attr__( 'Weekly', 'bon-toolkit' ), 
			'yearly'     => esc_attr__( 'Yearly', 'bon-toolkit' ) 
		);
		...
		$format = array( 
			'custom' => esc_attr__( 'Custom', 'bon-toolkit' ), 
			'html'   => esc_attr__( 'HTML', 'bon-toolkit' ), 
			'option' => esc_attr__( 'Option', 'bon-toolkit' ) 
		);
		?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>"><code><?php _e('Posts per page','bon-toolkit'); ?></code></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'type' ); ?>"><code><?php _e('Type','bon-toolkit'); ?></code></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'format' ); ?>"><code><?php _e('Format','bon-toolkit'); ?></code></label> 
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'before' ); ?>"><code><?php _e('Before','bon-toolkit'); ?></code></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'after' ); ?>"><code><?php _e('After','bon-toolkit'); ?></code></label>
		...
		<p>
			...
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_post_count'], true ); ?> id="<?php echo $this->get_field_id( 'show_post_count' ); ?>" name="<?php echo $this->get_field_name( 'show_post_count' ); ?>" /> <?php _e( 'Show post count?', 'bon-toolkit' ); ?> <code>show_post_count</code></label>
		</p>
		</div>
