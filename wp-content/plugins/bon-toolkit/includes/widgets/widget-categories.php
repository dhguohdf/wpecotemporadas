...
add_action( 'widgets_init', 'load_bon_toolkit_categories_widget' );

function load_bon_toolkit_categories_widget() {
	register_widget( 'Bon_Toolkit_Widget_Categories' );
}

class Bon_Toolkit_Widget_Categories extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			...
			'description' => esc_html__( 'An advanced widget that gives you total control over the output of your category links.', 'bon-toolkit' )
		);
		...
		/* Create the widget. */
		$this->WP_Widget(
			'bon-toolkit-categories',               // $this->id_base
			BON_TOOLKIT_PREFIX . ' ' . __( 'Categories', 'bon-toolkit' ), // $this->name
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title'              => esc_attr__( 'Categories', 'bon-toolkit' ),
			'taxonomy'           => 'category',
			'style'              => 'list',
			'include'            => '',
			'exclude'            => '',
		...
		$style = array( 
			'list' => esc_attr__( 'List', 'bon-toolkit' ), 
			'none' => esc_attr__( 'None', 'bon-toolkit' ) 
		);
		...
		$orderby = array( 
			'count'      => esc_attr__( 'Count', 'bon-toolkit' ), 
			'ID'         => esc_attr__( 'ID', 'bon-toolkit' ), 
			'name'       => esc_attr__( 'Name', 'bon-toolkit' ), 
			'slug'       => esc_attr__( 'Slug', 'bon-toolkit' ), 
			'term_group' => esc_attr__( 'Term Group', 'bon-toolkit' ) 
		);

		$feed_type = array( 
			''     => '', 
			'atom' => esc_attr__( 'Atom', 'bon-toolkit' ), 
			'rdf'  => esc_attr__( 'RDF', 'bon-toolkit' ), 
			'rss'  => esc_attr__( 'RSS', 'bon-toolkit' ), 
			'rss2' => esc_attr__( 'RSS 2.0', 'bon-toolkit' ) 
		);

		?>

		<div class="bon-toolkit-widget-controls columns-3">
		<p>
		...
		<p>
			...
			<input class="checkbox" type="checkbox" <?php checked( $instance['hierarchical'], true ); ?> id="<?php echo $this->get_field_id( 'hierarchical' ); ?>" name="<?php echo $this->get_field_name( 'hierarchical' ); ?>" /> <?php _e( 'Hierarchical?', 'bon-toolkit' ); ?> <code>hierarchical</code></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'use_desc_for_title' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['use_desc_for_title'], true ); ?> id="<?php echo $this->get_field_id( 'use_desc_for_title' ); ?>" name="<?php echo $this->get_field_name( 'use_desc_for_title' ); ?>" /> <?php _e( 'Use description?', 'bon-toolkit' ); ?> <code>use_desc_for_title</code></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'show_count' ); ?>">
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_count'], true ); ?> id="<?php echo $this->get_field_id( 'show_count' ); ?>" name="<?php echo $this->get_field_name( 'show_count' ); ?>" /> <?php _e( 'Show count?', 'bon-toolkit' ); ?> <code>show_count</code></label>
		</p>
		<p>
