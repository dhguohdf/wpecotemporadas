...
add_action( 'widgets_init', 'load_bon_toolkit_tag_widget' );

function load_bon_toolkit_tag_widget() {
	register_widget( 'Bon_Toolkit_Widget_Tags' );
}
 ...
 * @since 1.0
 */
class Bon_Toolkit_Widget_Tags extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'tags',
			'description' => esc_html__( 'An advanced widget that gives you total control over the output of your tags.', 'bon-toolkit' )
		);
		...
		/* Create the widget. */
		$this->WP_Widget(
			'bon-toolkit-tags',               // $this->id_base
			BON_TOOLKIT_PREFIX . ' ' . __( 'Tags', 'bon-toolkit' ), // $this->name
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title'                      => esc_attr__( 'Tags', 'bon-toolkit' ),
			'order'                      => 'ASC',
			'orderby'                    => 'name',
			'format'                     => 'flat',
			'include'                    => '',
		...
		$link = array( 
			'view' => esc_attr__( 'View', 'bon-toolkit' ), 
			'edit' => esc_attr__( 'Edit', 'bon-toolkit' ) 
		);

		$format = array( 
			'flat' => esc_attr__( 'Flat', 'bon-toolkit' ), 
		...
		<p>
			...
			<input class="checkbox" type="checkbox" <?php checked( $instance['pad_counts'], true ); ?> id="<?php echo $this->get_field_id( 'pad_counts' ); ?>" name="<?php echo $this->get_field_name( 'pad_counts' ); ?>" /> <?php _e( 'Pad counts?', 'bon-toolkit' ); ?> <code>pad_counts</code></label>
		</p>
		<p>
