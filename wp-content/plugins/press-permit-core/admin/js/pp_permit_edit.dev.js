jQuery(document).ready( function($) {
	...
	$(document).on( 'click', "#pp_tbl_role_selections .pp_clear", function(e) {
		var eid = $(this).closest('tr').find('input[name="pp_eid[]"]').val();

		if ( typeof all_roledata[eid] != 'undefined' ) {
			delete all_roledata[eid];
		}
	...
	$('#pp_add_site_role').bind('click', function() {
		...
		$(conds).each(function(){
			...
			if ( $.inArray( trackdata, all_roledata ) != -1 ) {
				duplicate = true;
			} else {
				eid++;
				all_roledata[eid]=trackdata;
				...
				newrow = '<tr>' 
						...
						+ '<input type="hidden" name="pp_add_role[' + eid + '][attrib_cond]" value="' + $('#' + id).val() + '" />';
						+ '</td>'
						+ '</tr>';
	...
	var reload_roles = function() {
		if ( $('select[name="pp_select_type"]').val() )
			ajax_ui('get_role_options',draw_roles);
		else
			$('.pp-select-role').hide();
	}
	
	var reload_conditions = function() {
		if ( $('select[name="pp_select_type"]').val() && $('select[name="pp_select_role"]').val() )
			ajax_ui('get_conditions_ui',draw_conditions);
		else
			$('.pp-select-cond').hide();
	}

	$('select[name="pp_select_role"]').bind( 'change', reload_conditions );
	...
	var draw_roles = function(data,txtStatus) {
	...
	var draw_conditions = function(data,txtStatus) {
		dv = $('#pp_cond_ui');
		dv.html(data);
	...
	var ajax_ui = function(op,handler,item_id) {
		...
		var data = { 'pp_ajax_ui': op, 'pp_src_name': 'post', 'pp_object_type' : $('select[name="pp_select_type"]').val(), 'pp_role_name': $('select[name="pp_select_role"]').val(), 'pp_item_id' : item_id };
		$.ajax({url:ppCred.ajaxurl, data:data, dataType:"html", success:handler, error:ajax_ui_failure});
	}
	
	var ajax_ui_done = function() {
	...
	var current_roles_ajax_done = function() {
		$('#pp_current_roles input.submit-edit-item-role').prop('disabled',false);
		$('#pp_current_roles .waiting').hide();
	}
	
	var remove_roles_done = function(data,txtStatus) {
		current_roles_ajax_done();
	...
	$('#pp_current_roles input.submit-edit-item-role').click( function(e) {
		...
		switch( action ) {
			case 'remove':
				ajax_submit('roles_remove',remove_roles_done,selected_ids.join('|'));
				break
			default:
				break
		}
	...
	var ajax_submit = function(op,handler,ass_ids) {
		if ( ! ass_ids )
			return;

		var data = { 'pp_ajax_submission': op, 'agent_type': ppCred.agentType, 'agent_id': ppCred.agentID, 'pp_ass_ids' : ass_ids };
		$.ajax({url:ppCred.ajaxurl, data:data, dataType:"html", success:handler, error:ajax_submit_failure});
	}
