function pp_load_agents_js( id_sfx, agent_type, context, agent_id, suppress_selection_js ) {
...
function pp_select_agents( id_sfx ) {
	jQuery(document).ready( function($) {
		var agent_ids = '';
		...
		pp_eagent_ajax_request(agent_ids, id_sfx);
	});
}

var pp_eagent_ajax_request= function(agent_ids,id_sfx) {
	jQuery(document).ready( function($) {
		var data = { 'pp_ajax_item_ui': 'get_agent_exception_ui',
		...
		$.ajax({url:ppException.ajax_url, data:data, dataType:"html", success:pp_eagent_got_ajax_listbox, error:pp_eagent_ajax_failure});
	});
}

var pp_eagent_got_ajax_listbox = function(data,txtStatus,req) {
	jQuery(document).ready( function($) {
...
var pp_eagent_ajax_failure = function(XMLHttpRequest, textStatus, errorThrown) {
	/*
