...
require_once( dirname(__FILE__).'/db-config_pp.php');
	
class PP_DB_Setup {
	public static function db_setup( $last_db_ver = false ) {
		global $pp_db_setup_done;
		
		if ( $last_db_ver && version_compare( $last_db_ver, '2.0.1', '<' ) ) {
			self::drop_alpha_indexes();
		}
	...
	public static function update_schema() {
		...
		//
			...
			role_name potentially contains "base_rolename:src_name:type_or_taxonomy:attribute:condition"
												(<=40)	  (<=32)		(<=32)		 (<=32)	   (<=32)		
	...
	function drop_alpha_indexes() {
		...
		$arr_sql = array(	
			"DROP INDEX `pp_exc2agent` ON $wpdb->ppc_exceptions",
			"DROP INDEX `pp_exc2item` ON $wpdb->ppc_exceptions",
			"DROP INDEX `pp_inherited` ON $wpdb->ppc_exception_items",
			"DROP INDEX `pp_exception_item` ON $wpdb->ppc_exception_items",
		);
 ...
 * @return unknown
 */
function _pp_dbDelta($queries, $execute = true) {	// lifted from MU 2.8.4a because forced inclusion of schema.php by Press Permit interferes with site creation
	global $wpdb;
