...
class GFEntryList{
    ...
    public static function leads_page($form_id){
        ...
        if($star)
            $search_criteria["is_starred"] =  (bool) $star;
        if($read)
            $search_criteria["is_read"] =  (bool) $read;

        $search_field_id = rgget("field_id");
        $search_operator = rgget("operator");
        $search_type = rgget("type");
        ...
        $search_type_qs = empty($search_type) ? "" : "&type=$search_type";
        $search_operator_qs = empty($search_operator) ? "" : "&operator=$search_operator";

        $display_total = ceil($total_count / $page_size);
        $page_links = paginate_links( array(
            'base' =>  admin_url("admin.php") . "?page=gf_entries&view=entries&id=$form_id&%_%" . $search_qs . $sort_qs . $dir_qs. $star_qs . $read_qs . $filter_qs . $search_field_id_qs . $search_type_qs . $search_operator_qs,
            'format' => 'paged=%#%',
        ...
        $freeform_filter = array(
            "key"=> "0",
            "type" =>  "global",
            "text" => __("Any field", "gravityforms"),
            "operators" => array("contains", "is")
        );
        array_unshift($field_filters,$freeform_filter);
        ...
        <script type="text/javascript">
            ...
            var messageTimeout = false,
                ...
                gformInitFilter = <?php echo json_encode($init_filter) ?>
            ...
            function Search(sort_field_id, sort_direction, form_id, search, star, read, filter, field_id, operator, type){
                ...
                var type_qs = type == "" ? "" : "&type=" + type;

                var location = "?page=gf_entries&view=entries&id=" + form_id + "&sort=" + sort_field_id + "&dir=" + sort_direction + search_qs + star_qs + read_qs + filter_qs + field_id_qs + operator_qs + type_qs;
            ...
            function BulkResendNotifications(){
                ...
                jQuery.post(ajaxurl, {
                    ...
                    type: '<?php echo esc_attr(rgget("type")) ?>',
            ...
            function BulkPrint(){
                ...
                var searchTypeQS = '&type=<?php echo esc_attr(rgget("type")) ?>';
                var searchOperatorQS = '&operator=<?php echo esc_attr(rgget("operator")) ?>';

                var url = '<?php echo trailingslashit(site_url()) ?>?gf_page=print-entry&fid=<?php echo $form['id'] ?>' + leadsQS + notesQS + pageBreakQS + filterQS + searchQS + searchFieldIdQS + searchTypeQS + searchOperatorQS;
            ...
            jQuery(document).ready(function(){
                ...
                jQuery("#entry_filters").on("keypress", ".gform-filter-value", (function(event){
                    if(event.keyCode == 13){
                        Search('<?php echo $sort_field ?>', '<?php echo $sort_direction ?>', <?php echo $form_id ?>, jQuery('.gform-filter-value').val(), '<?php echo $star ?>', '<?php echo $read ?>', '<?php echo $filter ?>', jQuery('.gform-filter-field').val(), jQuery('.gform-filter-operator').val(), jQuery('.gform-filter-type').val());
        ...
        <div class="wrap">
            ...
            <form id="lead_form" method="post">
                ...
                <div style="margin-top:12px;float:right;">
                    <a style="float:right;" class="button" id="lead_search_button" href="javascript:Search('<?php echo $sort_field ?>', '<?php echo $sort_direction ?>', <?php echo $form_id ?>, jQuery('.gform-filter-value').val(), '<?php echo $star ?>', '<?php echo $read ?>', '<?php echo $filter ?>', jQuery('.gform-filter-field').val(), jQuery('.gform-filter-operator').val(), jQuery('.gform-filter-type').val());"><?php _e("Search", "gravityforms") ?></a>
                ...
                <thead>
                    <tr>
                        ...
                        foreach($columns as $field_id => $field_info){
                            ...
                            ?>
                            <th scope="col" class="manage-column entry_nowrap" onclick="Search('<?php echo $field_id ?>', '<?php echo $dir ?>', <?php echo $form_id ?>, '<?php echo $search ?>', '<?php echo $star ?>', '<?php echo $read ?>', '<?php echo $filter ?>', '<?php echo $search_field_id ?>', '<?php echo $search_operator ?>', '<?php echo $search_type ?>');" style="cursor:pointer;"><?php echo esc_html($field_info["label"]) ?></th>
                            <?php
                        }
                        ?>
                        <th scope="col" align="right" width="50">
                            <a title="<?php _e("click to select columns to display" , "gravityforms") ?>" href="<?php echo trailingslashit(site_url()) ?>?gf_page=select_columns&id=<?php echo $form_id ?>&TB_iframe=true&height=365&width=600" class="thickbox entries_edit_icon"><i class="icon-cog"></i></a>
                        </th>
                    </tr>
                </thead>
                <tfoot>
                ...
                <tbody data-wp-lists="list:gf_entry" class="user-list" id="gf_entry_list">
                    ...
                    if(sizeof($leads) > 0){
                        ...
                        foreach($leads as $position => $lead){
                            ...
                            ?>
                            <tr id="lead_row_<?php echo $lead["id"] ?>" class='author-self status-inherit <?php echo $lead["is_read"] ? "" : "lead_unread" ?> <?php echo $lead["is_starred"] ? "lead_starred" : "" ?> <?php echo in_array($filter, array("trash", "spam")) ? "lead_spam_trash" : "" ?> <?php $gf_entry_locking->list_row_class($lead["id"]); ?>'  valign="top" data-id="<?php echo esc_attr($lead["id"]) ?>">
                                ...
                                foreach($field_ids as $field_id){
                                                ...
                                                foreach($lead_field_keys as $input_id){
                                                    ...
                                                    if(is_numeric($input_id) && absint($input_id) == absint($field_id)){
                                                        if($lead[$input_id] == $columns[$field_id]["label"]){
                                                            $value = "<i class='icon-ok gf_valid'></i>";
                                                        }
                                                        else{
                                                            $field = RGFormsModel::get_field($form, $field_id);
                                    ...
                                    if($is_first_column){
                                        ...
                                        <td class="column-title" >
                                            <a href="admin.php?page=gf_entries&view=entry&id=<?php echo $form_id ?>&lid=<?php echo $lead["id"] . $search_qs . $sort_qs . $dir_qs . $filter_qs?>&paged=<?php echo ($page_index + 1)?>&pos=<?php echo $position; ?>&field_id=<?php echo $search_field_id; ?>&type=<?php echo $search_type; ?>&operator=<?php echo $search_operator; ?>"><?php echo $value ?></a>
                                            ...
                                            <div class="row-actions">
                                                ...
                                                switch($filter){
                                                    case "trash" :
                                                        ?>
                                                        <span class="edit">
                                                            <a title="<?php _e("View this entry", "gravityforms"); ?>" href="admin.php?page=gf_entries&view=entry&id=<?php echo $form_id ?>&lid=<?php echo $lead["id"] . $search_qs . $sort_qs . $dir_qs . $filter_qs?>&paged=<?php echo ($page_index + 1)?>&pos=<?php echo $position; ?>&field_id=<?php echo $search_field_id; ?>&type=<?php echo $search_type; ?>&operator=<?php echo $search_operator; ?>"><?php _e("View", "gravityforms"); ?></a>
                                                            |
                                                        </span>
