...
class GFFormsModel {
    ...
    public static function get_form($form_id){
        ...
        $results = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE id=%d AND is_trash = 0", $form_id));
    ...
    */
    ...
    public static function search_lead_ids($form_id, $search_criteria = array()) {
        ...
        $field_filters_where = self::get_field_filters_where($form_id, $field_filters);
    ...
    private static function get_field_filters_where($form_id, $field_filters) {
        global $wpdb;

        if(empty($field_filters))
            return;

        $search_operator = isset($field_filters["mode"]) && $field_filters["mode"] == "any" ? "OR" : "AND";
    ...
    private static function get_info_search_where($search_criteria) {
        ...
        foreach ($search_criteria as $key=>$value) {
            ...
            if(in_array($key, $int_columns)){
                $where_array[] = $wpdb->prepare("l.{$key} = %d", $value);
                continue;
            }

            switch ($key){
                case "start_date" :
                    $where_array[] = $wpdb->prepare("datediff(date_created, %s) >= 0", $value);
                break;
                case "end_date" :
                    $where_array[] = $wpdb->prepare("datediff(date_created, %s) <= 0", $value);
                break;
                ...
                default :
                    $where_array[] = $wpdb->prepare("l.{$key} = %s", $value);
