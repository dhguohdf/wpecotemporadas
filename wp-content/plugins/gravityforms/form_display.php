...
class GFFormDisplay{
    ...
    public static function enqueue_form_scripts($form, $ajax=false){
        ...
        if( self::has_price_field( $form ) || self::has_password_strength( $form ) || GFCommon::has_list_field( $form ) || GFCommon::has_credit_card_field($form) || self::has_conditional_logic( $form ) || self::has_calculation_field( $form ) ) {
    ...
    public static function print_form_scripts($form, $ajax){
        ...
        if( (self::has_enhanced_dropdown($form) || self::has_price_field($form) || self::has_password_strength($form) || self::has_pages($form) || self::has_password_strength($form) || GFCommon::has_list_field($form) || GFCommon::has_credit_card_field($form)) && !wp_script_is("gform_gravityforms", "queue")){
            wp_enqueue_script("gform_gravityforms", GFCommon::get_base_url() . "/js/gravityforms.js", array("jquery"), GFCommon::$version, false);
            wp_print_scripts(array("gform_gravityforms"));
        }

        if( self::has_conditional_logic( $form ) && ! wp_script_is( 'gform_conditional_logic' ) ) {
            wp_enqueue_script( 'gform_conditional_logic' );
            wp_print_scripts( array( 'gform_conditional_logic' ) );
        }

        if( self::has_datepicker_field( $form ) && ! wp_script_is( 'gform_datepicker_init' ) ){
            wp_enqueue_script( 'gform_datepicker_init' );
            wp_print_scripts( array( 'gform_datepicker_init' ) );
        }

        if( self::has_pages( $form ) && ! wp_script_is( 'gform_json' ) ) {
            wp_enqueue_script( 'gform_json' );
            wp_print_scripts( array( 'gform_json' ) );
        }

        if( (self::has_enhanced_dropdown($form) || self::has_price_field($form) || self::has_password_strength($form) || self::has_pages($form) || self::has_password_strength($form) || GFCommon::has_list_field($form) || GFCommon::has_credit_card_field($form)) || self::has_calculation_field($form) && !wp_script_is("gform_gravityforms", "queue")){
            wp_enqueue_script("gform_gravityforms", GFCommon::get_base_url() . "/js/gravityforms.js", array("jquery"), GFCommon::$version, false);
            wp_print_scripts(array("gform_gravityforms"));
        }
        ...
        if( self::has_character_counter( $form ) && ! wp_script_is( 'gform_textarea_counter' ) ) {
            wp_enqueue_script( 'gform_textarea_counter' );
            wp_print_scripts( array( 'gform_textarea_counter' ) );
        }

        if( self::has_input_mask( $form ) && ! wp_script_is( 'gform_masked_input' ) ) {
            wp_enqueue_script( 'gform_masked_input' );
            wp_print_scripts( array( 'gform_masked_input' ) );
        }

        if( self::has_enhanced_dropdown( $form ) && ! wp_script_is( 'gform_chosen' ) ) {
            wp_enqueue_script( 'gform_chosen' );
            wp_print_scripts( array( 'gform_chosen' ) );
        }
    ...
    */
    ...
    public static function get_field_content($field, $value="", $force_frontend_label = false, $form_id=0){
        ...
        $duplicate_field_link = !in_array($field['type'], $duplicate_disabled) ? "<a class='field_duplicate_icon' id='gfield_duplicate_$id' title='" . __("click to duplicate this field", "gravityforms") . "' href='#' onclick='StartDuplicateField(this); return false;'><i class='icon-copy icon-large'></i></a>" : "";
        $duplicate_field_link = apply_filters("gform_duplicate_field_link", $duplicate_field_link);

        $delete_field_link = "<a class='field_delete_icon' id='gfield_delete_$id' title='" . __("click to delete this field", "gravityforms") . "' href='#' onclick='StartDeleteField(this); return false;'><i class='icon-remove icon-large'></i></a>";
        ...
        $admin_buttons = IS_ADMIN ? "<div class='gfield_admin_icons'><div class='gfield_admin_header_title'>{$field_type_title} : " . __("Field ID", "gravityforms") . " {$field["id"]}</div>" . $delete_field_link . $duplicate_field_link . "<a class='field_edit_icon edit_icon_collapsed' title='" . __("click to expand and edit the options for this field", "gravityforms") . "'><i class='icon-caret-down icon-large'></i></a></div>" : "";
