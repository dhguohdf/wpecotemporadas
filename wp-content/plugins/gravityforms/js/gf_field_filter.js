(function (gfFieldFilterUI, $) {
    ...
    $.fn.gfFilterUI = function(filterSettings, initFilters, allowMultiple, minResizeHeight) {
        init(this, filterSettings, initFilters, allowMultiple, minResizeHeight );
        return this;
    };

    // private
    var $container, operatorStrings, settings, filters, imagesURL, isResizable, allowMultiple, height;

    function init (c, s, f, m, h){
        $container = $(c);
        $container
    ...
    function getFilterFields() {
        var i, j, key, val, label, question, options = "", disabled = "", numRows,
            str = "<select class='gform-filter-field' name='f[]' onchange='gfFilterUI.changeField(this)'>";
        for (i = 0; i < settings.length; i++) {
            ...
            if (settings[i].type == 'group') {
                ...
                for (j = 0; j < numRows; j++) {
                    ...
                    options += '<option {0} value="{1}">{2}</option>'.format(disabled, val, label);
                }
                str += '<optgroup label="{0}">{1}</optgroup>'.format(question, options);
            } else {
                disabled = settings[i].preventMultiple && isFieldSelected(key) ? "disabled='disabled'" : "";
                label = settings[i].text;
                str += '<option {0} value="{1}">{2}</option>'.format(disabled, key, label);
            }

        }
        str += "</select>";
        str += "<input type='hidden' class='gform-filter-type' name='t[]' value='' >";
        return str;
    }
    ...
    function addFilterMode ($filterRow) {
        var html;
        html = '<select name="mode"><option value="all">{0}</option><option value="any">{1}</option></select>'.format(gf_vars.all, gf_vars.any);
