...
var GFCalc = function(formId, formulaFields){
    ...
    this.runCalc = function(formulaField, formId) {
        ...
        expr = calcObj
                .replaceFieldTags( formId, formulaField.formula, formulaField )
                .replace(/(\r\n|\n|\r)/gm,"");
        ...
        formattedResult = gform.applyFilters( 'gform_calculation_format_result', result, formulaField, formId, calcObj );
    ...
    function setup(uploadElement){
        ...
        uploader.bind('FilesAdded', function(up, files) {
            ...
            $.each(files, function(i, file) {
                ...
                $('#' + up.settings.filelist).prepend(
                    '<div id="'
                        + file.id
                        + '" class="ginput_preview">'
                        + file.name
                        + ' (' + size + ') <b></b> '
                        + '<a href="javascript:void(0)" title="' + strings.cancel_upload + '" onclick=\'$this=jQuery(this); var uploader = gfMultiFileUploader.uploaders.' + up.settings.container + ';uploader.stop();uploader.removeFile(uploader.getFile("' + file.id +'"));$this.after("' + strings.cancelled + '"); uploader.start();$this.remove();\'>' + strings.cancel + '</span>'
                  + '</div>');
                totalCount++;
        ...
        uploader.bind('FileUploaded', function(up, file, result) {
            ...
            if(file.percent == 100){
                ...
                if(response.data.temp_filename === tempFileName && response.data.uploaded_filename === file.name)
