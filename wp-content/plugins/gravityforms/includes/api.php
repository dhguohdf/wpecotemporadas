...
class GFAPI {
    ...
    public static function get_form($form_id) {
        if (!GFCommon::current_user_can_any("gravityforms_edit_forms"))
            return new WP_Error("not_allowed", __("You don't have adequate permissions to view form objects", "gravityforms"));
    ...
    public static function update_forms($forms) {
        ...
        if (!GFCommon::current_user_can_any("gravityforms_edit_forms"))
            return new WP_Error("not_allowed", __("You don't have adequate permissions to update forms.", "gravityforms"));
    ...
    public static function update_form($form_meta, $form_id = null) {
        ...
        if (0 === $result)
            return new WP_Error("not_found", sprintf(__("Form with id %s not found", "gravityforms"), $form_id));
    ...
    public static function add_form($form_meta) {
        ...
        if (!GFCommon::current_user_can_any("gravityforms_create_form"))
            return new WP_Error("not_allowed", __("You don't have adequate permissions to create forms.", "gravityforms"));
        ...
        if (false === $result)
            return new WP_Error("insert_form_error", __("There was a problem while inserting the form"), $wpdb->last_error);
     ...
     *     $search_criteria["currency"] = "USD";
     *     $search_criteria["is_read"] = true;
     *
     ...
     *     $search_criteria["field_filters"][] = array('type' => 'field', 'key' => "1", 'value' => "gquiz159982170");
     *
     *  Filter by a checkbox value (not recommended)
     *     $search_criteria["field_filters"][] = array('type' => 'field', 'key' => "2.2", 'value' => "gquiz246fec995");
     ...
     *     $search_criteria["field_filters"][] = array('type' => 'field', 'key' => "2", 'value' => "gquiz246fec995");
     *
     *  Filter by a global search of values of any field
     *     $search_criteria["field_filters"][] = array('type' => 'global', 'value' => $search_value);
     *
     *  Filter entries by Entry meta (added using the gform_entry_meta hook)
     *     $search_criteria["field_filters"][] = array('type' => 'meta', 'key' => "gquiz_score", 'value' => "1");
     *     $search_criteria["field_filters"][] = array('type' => 'meta', 'key' => "gquiz_is_pass", 'value' => "1");
     *
     *  Filter by a global search of values of any field
     *     $search_criteria["field_filters"][] = array('type' => 'global', 'value' => $search_value);
     *
    ...
    public static function get_entries($form_ids, $search_criteria = array(), $sorting = null, $paging = null, &$total_count = null) {
        ...
        if (!GFCommon::current_user_can_any("gravityforms_view_entries"))
            return new WP_Error("not_allowed", __("You don't have adequate permissions to view entries.", "gravityforms"));
    ...
    public static function get_entry($entry_id) {
        ...
        $search_criteria["id"] = $entry_id;
        $paging                = array("offset" => 0, "page_size" => 1);
        $entries               = self::get_entries(0, $search_criteria, null, $paging);

        if (empty($entries))
            return new WP_Error("not_found", sprintf(__("Entry with id %s not found", "gravityforms"), $entry_id));
    ...
    public static function update_entries($entries) {
        ...
        if (!GFCommon::current_user_can_any("gravityforms_edit_entries"))
            return new WP_Error("not_allowed", __("You don't have adequate permissions to update entries.", "gravityforms"));
    ...
    public static function update_entry($entry, $entry_id = null) {
        ...
        if(!$current_entry)
            return new WP_Error("not_found", __("Entry not found", "gravityforms"));
        ...
        $payment_status = isset($entry["payment_status"]) ? mysql_real_escape_string($entry["payment_status"]) : 'NULL';
        $payment_date   = strtotime(rgar($entry, "payment_date")) ? "'" . gmdate('Y-m-d H:i:s', strtotime("{$entry["payment_date"]}")) . "'" : "NULL";
        $payment_amount = isset($entry["payment_date"]) ? (float)$entry["payment_amount"] : 'NULL';
        $transaction_id = isset($entry["transaction_id"]) ? mysql_real_escape_string($entry["transaction_id"]) : 'NULL';
        $is_fulfilled   = isset($entry["is_fulfilled"]) ? intvat($entry["is_fulfilled"]) : 'NULL';
        ...
        $result     = $wpdb->query($wpdb->prepare("
                ...
                WHERE
                id = %d
                ", $form_id, $is_starred, $is_read, $ip, $source_url, $user_agent, $currency, $entry_id));
        if (false === $result)
            return new WP_Error("update_entry_properties_failed", __("There was a problem while updating the entry properties"), $wpdb->last_error);
            ...
            if (isset($field["inputs"]) && is_array($field["inputs"])) {
                foreach ($field["inputs"] as $input) {
                    ...
                    if (isset($entry[$input_id])) {
                        ...
                        if(false === $result)
                            return new WP_Error("update_input_value_failed", __("There was a problem while updating one of the input values for the entry"), $wpdb->last_error);
                ...
                if (false === $result)
                    return new WP_Error("update_field_values_failed", __("There was a problem while updating the field values"), $wpdb->last_error);
    ...
    public static function add_entry($entry) {
        ...
        if (!GFCommon::current_user_can_any("gravityforms_edit_entries"))
            return new WP_Error("not_allowed", __("You don't have adequate permissions to add entries.", "gravityforms"));
        ...
        $result     = $wpdb->query($wpdb->prepare("
                ...
                (form_id, post_id, date_created, is_starred, is_read, ip, source_url, user_agent, currency, payment_status, payment_date, payment_amount, transaction_id, is_fulfilled, created_by, transaction_type)
                VALUES
                (%d, {$post_id}, {$date_created}, %d,  %d, %s, %s, %s, %s, {$payment_status}, {$payment_date}, {$payment_amount}, {$transaction_id}, {$is_fulfilled}, {$user_id}, {$transaction_type})
                ", $form_id, $is_starred, $is_read, $ip, $source_url, $user_agent, $currency));
        if (false === $result)
            return new WP_Error("insert_entry_properties_failed", __("There was a problem while inserting the entry properties"), $wpdb->last_error);
            ...
            if (isset($field["inputs"]) && is_array($field["inputs"])) {
                foreach ($field["inputs"] as $input) {
                    ...
                    if (isset($entry[(string)$input_id])) {
                        ...
                        if(false === $result)
                            return new WP_Error("insert_input_value_failed", __("There was a problem while inserting one of the input values for the entry"), $wpdb->last_error);
                    }
                }
            } else {
                $field_id = $field["id"];
                ...
                if (false === $result)
                    return new WP_Error("insert_field_values_failed", __("There was a problem while inserting the field values"), $wpdb->last_error);
            }
        }
