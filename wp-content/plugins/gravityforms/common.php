...
class GFCommon{
    ...
    public static function get_version_info($cache=true){
        ...
        if(!$raw_response){
            ...
            $request_url = GRAVITY_MANAGER_URL . "/version.php";
    ...
    public static function get_remote_post_params(){
        ...
        foreach ($plugin_list as $key => $plugin) {
        	...
        	if (strtolower($plugin["Author"]) == "rocketgenius"){
    	...
    	$post = array("of" => "gravityforms", "key" => self::get_key(), "v" => self::$version, "wp" => get_bloginfo("version"), "php" => phpversion(), "mysql" => $wpdb->db_version(), "version" => "2", "plugins" => $plugins, "tn" => $theme_name, "tu" => $theme_uri, "tv" => $theme_version, "ta" => $theme_author, "tau" => $theme_author_uri, "im" => $im, "fc" => $fc, "ec" => $entry_count, "emc" => self::get_emails_sent());
    	return $post;
    }
    ...
    public static function get_field_input($field, $value="", $lead_id=0, $form_id=0){
        ...
        switch(RGFormsModel::get_input_type($field)){
            ...
            case "fileupload" :
                ...
                if($multiple_files){
                    ...
                    $plupload_init = defined('DOING_AJAX') && DOING_AJAX && "rg_change_input_type" === rgpost('action') ? array() : array(
                        ...
                        'filters' => array( array('title' => __( 'Allowed Files' ), 'extensions' => $allowed_extensions) ),
                        'multipart' => true,
            ...
            case "creditcard" :
                ...
                $card_field =   sprintf("<span class='ginput_full{$class_suffix}' id='{$field_id}_1_container' >{$card_icons}<input type='{$html_input_type}' name='input_%d.1' id='%s_1' value='%s' {$tabindex} %s {$onchange} {$onkeyup} {$autocomplete} /><label for='%s_1' id='{$field_id}_1_label'>" . apply_filters("gform_card_number_{$form_id}", apply_filters("gform_card_number",__("Card Number", "gravityforms"), $form_id), $form_id) . "</label></span>", $id, $field_id, $card_number, $disabled_text, $field_id);
    ...
    public static function localize_gform_gravityforms_multifile(){
        wp_localize_script( 'gform_gravityforms', 'gform_gravityforms', array(
            'strings' => array(
                ...
                "cancelupload" => __("Cancel this upload", "gravityforms"),
