...
class Limit_Image_Upload {
	...
	public function limit_handle_upload_prefilter( $file ) {
		...
		if ( $this->get_count_attachments() > self::$limit_upload - 1 ) {
			...
			$file['error'] = sprintf( 
				__( 'Desculpe, você não pode jogar mais que  %d imagens.', self::$textdomain ),
				self::$limit_upload
			);
		}
	...
	public function restrict_mime_types_hint(){
		...
		echo '<br />';
		_e( ' Insira apenas imagens:', self::$textdomain );
		echo ' <strong>png, jpg, pdf, bmp, gif e tiff.</strong>';
	}
